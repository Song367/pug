...
func anonPersonsCTE(projectID string) *chq.Query {
	return chq.NewQuery().
		Select(
			...
			"argMaxIfMerge(states.latest_web_browser_state) AS latest_browser",
			"argMaxIfMerge(states.latest_web_browser_version_state) AS latest_browser_version",
			"argMaxIfMerge(states.latest_web_os_state) AS latest_os",
			"argMaxIfMerge(states.latest_web_os_version_state) AS latest_os_version",
			"argMaxIfMerge(states.latest_web_device_state) AS latest_device",
			"argMaxIfMerge(states.latest_web_country_state) AS latest_country",
			"argMaxIfMerge(states.latest_web_region_state) AS latest_region",
			"argMaxIfMerge(states.latest_web_city_state) AS latest_city",
		).
